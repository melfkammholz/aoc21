...
void test_case() {
  ...
  vvi v(g.size(), vi(g[0].size()));
  vector<ii> dirs = DIR4;
  auto in = [&](int y, int x, int dy, int dx) {
    return y + dy >= 0 && y + dy < SZ(g) && x + dx >= 0 && x + dx < SZ(g[y]);
  };
  ...
  function<int(int, int)> dfs = [&](int y, int x) {
    ...
    int s = 1;
    for (auto [dy, dx] : dirs) {
        if (!in(y, x, dy, dx)) continue;
        if (v[y + dy][x + dx]) continue;
        if (g[y + dy][x + dx] == 9) continue;
        s += dfs(y + dy, x + dx);  
    }
    return s;
  };

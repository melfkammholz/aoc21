...
void test_case() {
  ...
  auto eraseChar = [](string& s, char c) {
  ...
  while (getline(cin, s)) {
    ...
    while (ss >> t) {
      SORT(t);
      if (t.size() == 2) m1[t] = 1, m2[1] = t;
      else if (t.size() == 4) m1[t] = 4, m2[4] = t;
      else if (t.size() == 3) m1[t] = 7, m2[7] = t;
      else if (t.size() == 7) m1[t] = 8, m2[8] = t;
      else if (t.size() == 5) A.insert(t);
      else if (t.size() == 6) B.insert(t);
      else break;
    }

    auto deduce = [&](int a, int b, int c, set<string>& s) {
      string v;
      for (string u : s) {
        string t = m2[a];
        for (char c : u)
